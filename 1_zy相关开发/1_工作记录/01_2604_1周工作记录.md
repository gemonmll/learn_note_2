## 0413 学习记录
1) c801 代码 ota 协议适配 
2) git 操作学习使用（cherry pick, git status ,git branch） ： 注意在从云端拉代码时直接用自己分支的名字，这样会在云端前直接加上origin
3) aep 编译，cicd工具学习使用
4) dkit工具学习使用上板测试
5) wireshark someip 协议抓包 (lua,host) 参考链接：https://zhuanlan.zhihu.com/p/22170327605?share_code=1m6uFH8qyZ7XB&utm_psn=2027076650790130147
6) git pr 操作： 注意选择人（需求者，预集成，通信负责人，诊断负责人）



## 0414 学习记录
1) 独立下载功能理解
2）两套通信（comif dcms模块理解）


## 0416 学习记录
御哥分支
![alt text](image.png)

1）关注diag uds上传机制
理解各个模块
2）![alt text](image-1.png)

3）git push -u/--up-stream 参数含义，追踪远程仓库

## 0417 学习记录
1）cicd 编译（加 --Production指令）
2）dkit 下载小包 直接加--file 会从云上下载小包
3）量产模式切换(正常模式与debug模式)
![alt text](image-2.png)
4）烧录大包，需要切换（carmode 传感器mode syu）
5) 救急板，还需要导入证书，和切换模式
6）板子log日志地址 /mnt/home/root#cd /mnt/dj/partitions/user/dlog/data0/txtlog-192.168.1.101/data0/
7) had地址（172.20.0.68 用于和其他对手件通信， 192.168.1.101 调试地址root用户）
8）黑匣子地址 192.168.1.111 用户dji
9) 实车wifi密码： Br1609dji
10）git commit 和commit(amend)使用区别、git fetch\git pull区别、当冲突时如何使用本地模型
11）注意commit 注释格式
12）烧录打包注意 使用的包和板子一样，普通开发最好使用明文包osq
![alt text](image-3.png)
![alt text](image-5.png)

## 疑问点
1）独立下载校验过程，是否是一口气校验
2）cicd 编译选项应该选择哪些 如何应该根据分支编译,应该选择哪个
![alt text](image-4.png)
3）板子冒烟测试

## 0418 学习记录
1）windows下wsl工具安装 （注意配置网络 mirror网络，开启复杂网络选项，安装docker 配置systemmd）
2）vn5620工具学习使用(host casc口扩展口)
3) someip协议详细研究

### 0420 学习记录
1）学习git add 和git commit ，理解git 工作区、暂存区和提交区、理解changes和stage changes， git pr流程、restore和reset、还有cherrypick，git rebase和git merge
2) 101 root had板子、111 dji 黑匣子、wifi密码 BR1609dji
3) 诊断日志地址：txt-bsp.log 和txt.log 通信日志地址：bin-mcu.log
黑匣子目录  
/tmp/where_is_dlog
/blackbox/car_data/car_2026-04-20_15-32-34/dlog/data0/txtlog-bsp-192.168.1.101/data0$ cat log |grep 
had目录
/mnt/dji/partutions/user/dlog/datao/txtlog-bsp
4) uds 1902 1906服务，通过dkit发送诊断指令
   发送 doip诊断指令：dkit diag send --ip 172.20.0.68 --proj faw --msg 1003
5) 导证书、切换debug模式，正常使用都是debug online模式
   导证书dkit cert install --cnt -1 --mode debug --start 2020-01-10 --end 2024-12-30 --proj faw --ip 172.20.0.68
   切模式dkit mode set --system debug_online --proj faw --ip 172.20.0.68
   
### 0421 学习记录
1）学习uds 1902 1906指令、理解uds前置条件表
dkit diag send --ip 172.20.0.68 --proj faw --msg 1902ff
dkit diag send --ip 172.20.0.68 --proj faw --msg 190696a002ff
dkit diag send --ip 172.20.0.68 --proj faw --msg 1906d47c8aff

2）理解cpu trap指令，特权态和非特权态
3）autosar各个模块间的通信机制，不理解（pdur can dcm）

4）systemdiag调用路径rte10ms DiagApp_MainFunction->

5）配合log文件SYS_SetDTC查看是否有faultid
.lst文件 用v来查看 还有后缀为xx用zcat查看



6)ecushell 打桩
![alt text](image-6.png)
export PATH=/mnt/dji/apps/dsar-app/bin/tools/:$PATH


2）板端phm测试，冒烟测试
172.200.68：19999 

1）e202-10属于哪个系列（p567还是e系列） --production=fawhq_e202_10 --debug-version应该怎么写
使用e202构建计划来编译，请教通信同事，另外可以查询其他人编译指令


### 0422 学习记录

1）dcm入口 dem入口DiagApp_MainFunction
2) 下载大包在分步版本规划中下载 切换车型 选择极限1261
3) 注意e202-10板子和c801p567板子不同
4) 查询他人编译指令 
cicd query buildCmd --repo-name=dsar-hq --user=wenguangma
5) 故障降级表
![alt text](image-7.png)
6）前置条件表
![alt text](image-8.png)
7）理解ecu_shell 宏，机制为注册进了一个section段，


## 0423 学习记录
1）冒烟测试工具，注意配置
2）someip仿真模拟测试，先启docker,再起板子，最后启动脚本
3）vscode 不同分支对比
4）e系列共包
5）git merge 操作有几点注意：1）比较共同祖先 2）income和current不能互相比较，都是比较共同祖先 3）有些不冲突的改动直接合入，需要git diff/compare查看不同，但从git commit不容易看到区别
6）git restore --staged --worktree . 恢复快照

## 0424学习记录
1）someip 表格接收发送区别（注意是否为server端）
2）someip 测试工具使用（注意防火墙）

## 0427学习记录
1）git lfs大文件上传 ，.gitattr 
2）利用git rebase 压缩 s\e\p
3) 传输大端 解析小端
4) 
![alt text](image-9.png)

## 0428学习记录
1）冒烟测试报考看bf相关（内存73md 使用率）
2）仿真测试共享知识库视频
3）整条通信链路有个ppt注意学习
5）一些自动生成的代码主要是合json，合入json后再生成代码合入、大文件使用cicd push
6）git history compare和beyond compare比较区别

```cpp
5、重启域控
6、上传小包至  /blackbox/furentao   (dsar/dsarapp)
7、升级小包
 
8、重启域控
9、查看版本，dsar/dar app version
10、查看log  
cat /tmp/where_is_dlog.txt
cd 至data0
调试log
dlog_viewer tail log.sl.zst | grep -aie “avm sts”
历史log
v log.sl.zst | grep -aie “avm sts”
v log.sl.zst | grep -aie “avm sts” -aie “avm enterst” -aie “avm actv” -aie “avm exit”
```

## 0428 学习记录
1）理解pnc8 nm网络管理（5c1 598）、理解ia和网管
2）canoe网络相关操作， link口（网络直连）、switch口（路由层）、vlan（mac层）向理解
3)
一汽can 网络管理part3 single nm
一汽can 网络管理part4 pnc nm

4)需要理解哨兵模式
4.1正常模式下pdc发5c1网管报文，为主节点，唤醒had,had收到5c1后被动唤醒，发送应用报文和网管598报文
4.2哨兵模式下：had为主控节点，发送598pnc8，pdc收到后外发5c1和pnc8，此时had为主节点，不受到5c1网管影响
4.3哨兵模式需要每隔几小时需要重启数据落盘，因此需要主动重启，重启过程是先正常发送应用报文，ia控制停发598pnc8,之后pdc收到停发的598pnc8后，也会停发5c1pnc8,但会有正常的5c1网管,当收到最后一帧5c1后1s中，所有控制器停发应用报文，5s后休眠。但此时had不应该休眠，ia控制had重启，重启后不发送应用报文和网管报文，等待ia上电启动，当ia启动后再有ia决定是否外发应用报文和网管报文。


```bash
哨兵重启测试手顺

1)网络唤醒had
断开ig硬线，canoe设备周期发送0x5c1网管报文（activeWakeupBit、ParialNetworkInformation、PNC8）设置为1

2)模拟哨兵重启(两种方式都可以模拟)
 mcu侧nm接口模拟重启
cd /mnt/dji/apps/dsar-app/bin/tools
./ecu_shell -d dsar_mcu
nm_service 4 4 0 0 0 （需卓驭老师确定，指令为nm_service 4 4 0 0 0还是 nm_service 4 3 0 0 0 ）

soc侧ia接口模拟重启
cd /mnt/dji/apps/dsar-app/bin/tools
./ecu_shell -d dsar_app
pal_item_set Cal_Sentry_NmCtrlVal 4
pal_item_set Cal_Sentry_CondMask 255
pal_item_set Cal_Sentry_CondVal 255
pal_item_set Cal_Sentry_Debug 2

3)指令输入后，外部停发5c1网管报文，观察板子是否重启及报文发送情况
```


 
## 0508 学习记录
1）大包能够读取版本号需要用到release版本才行，ota升级前需要切换normal模式
2）消息格式：had目前是3302,vin是LFPHC7PP6R1A60056,工程车
3）canoe eth配置 num选择2， simulation也要添加，measurement port勾选

## 0509 学习记录
1）ota升级需要用release zip包,学习彭哥发的zcat log*|grep up_master指令
![alt text](image-10.png)
2）下载前删除相关文件
3）注意使用--proj faw-rls来烧录release zip包

## 0511 学习记录
1) ecu shell 指令
cd /mnt/dji/apps/dsar-app/bin/tools/&&./dsar_shell 
可以后面加指令为 mcu 或是 app
![alt text](image-11.png)
2) log.sl.zst日志查看
   ![alt text](image-12.png)

## 0512 学习记录
1) usbft刷板子注意选择陪测包、正常使用172口还需要用192调试口刷两次大包
2) comif接口查看文档 红旗dsar-app从comif获取独立下载相关的someip信号
3）标定测试查看文档 红旗标定测试指南
 
## 0513 学习记录
1) dcms学习


## 0602 dssad使用学习
1）打开fl开关
fl_debug system debug_mode enable
fl_debug mock user_authorization on
2）查看落盘数据
 fl_debug trigger list
fl_debug trigger --package_id=xx
3)模拟故障
fl_debug mock trigger --id=1005



代办项点：
3）canoe网络相关操作， link口（网络直连）、switch口（路由层）、vlan（mac层）向理解
4)一汽can 网络管理part3 single nm 一汽can 网络管理part4 pnc nm 、理解pnc8 nm网络管理（5c1 598）、理解ia和网管
1) comif接口查看文档 红旗dsar-app从comif获取独立下载相关的someip信号
3）标定测试查看文档 红旗标定测试指南
