...
DKIT_WORK_DIR = Path(r"D:\dtf\plugins\dkit")
MAIN_OPTIONS = {
    "1": {
        "name": "DKIT 指令",
        "mode": "dkit",
...
def build_command_items(step, ip, file_value):
    ...
    for input_value in input_values:
        ...
        argv = split_command_args(argv_cmd, input_value)
...
def split_command_args(cmd, input_value=None):
    argv = shlex.split(cmd, posix=not IS_WINDOWS)
    if input_value is None:
        return argv

    return [input_value if arg == FILE_ARG_PLACEHOLDER else arg for arg in argv]


def ensure_dkit_work_dir():
    try:
        DKIT_WORK_DIR.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        print(f"dkit 工作目录不可用: {DKIT_WORK_DIR}, {e}")
        return None

    return str(DKIT_WORK_DIR)
...
def run_cmd_realtime(cmd, argv=None):
    ...
    try:
        if argv is None:
            argv = split_command_args(cmd)

        work_dir = ensure_dkit_work_dir()
        if work_dir is None:
            return False
