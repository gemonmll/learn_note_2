...
| 状态 | 判断逻辑 | 实现位置 |
|------|---------|---------|
| **collision** (碰撞) | 滑动窗口内 X或Y 方向加速度积分速度变化 > 8.0 km/h | dssad_data_get.c:289-298 |
| **lock** (锁车) | X方向速度增量 > 25.0 km/h 或 airbag crash signal = 1 | dssad_data_get.c:301-310 |
| **collision_risk** (碰撞风险) | AEB减速度 > 5.0 m/s² 或 SACC减速度 > 5.0 m/s² 或 InterventionRequest = 3 | dssad_data_get.c:277-286 |
...
| 方法 | 操作 |
|------|------|
| 查看 DSSAD 日志 | 事件触发时打印: `<DSSAD>dssad_time_stamp_event trigger, event_id:0x01...` |
| 订阅 DCMS topic | 用 `topic_recoder` 工具订阅输出 topic，确认有数据 |
| 查看 FL 日志 | FL 收到 `DCMS_TOPIC_FL_OPT_SEND` 时应打印存储日志 |
| 检查存储目录 | 取决于 FL 配置，通常在持久化分区 |
...
|-------|--------|---------|---------|
| 0 | Debug_None | (保留) | |
| 1 | Debug_ADS_Activate | ADS自动激活 | bit位对应32个子事件 |
| 2 | Debug_ADS_Exit | ADS自动退出 | bit位对应32个子事件 |
| 3 | Debug_InterventionRequest | 介入请求 | 0→1→2→3 递增 |
| 4 | Debug_MinimumRiskStrategy | 最小风险策略 | 0/1 |
| 5 | Debug_ADS_SeriousFailure | ADS严重失效 | bit位对应64个DTC |
| 6 | Debug_Collision | 碰撞 | 0/1 |
| 7 | Debug_RiskOfCollision | 碰撞风险 | 0/1 |
| 8 | Debug_ADS_ActivatedByDriver | 驾驶员激活 | 0/1 |
| 9 | Debug_ADS_ExitedByDriver | 驾驶员退出 | 0/1 |
| 10 | Debug_Lock | 锁车 | 0/1 |
...
```
步骤1: 开启调试模式
  send(/dssad/trigger_debug/v1, [0xFF, 0x01])
  预期: 日志打印 byte 位映射表

步骤2: 模拟 ADS 激活 (NOD子事件)
  send(/dssad/trigger_debug/v1, [1, 0x01])
  预期: 每50ms检测到 false→true
       日志: "<DSSAD>dssad_time_stamp_event trigger, event_id:0x01
               (EventId_ADS_Activate) subevent_id 0(Byte_NOD_Activate) ret 0"
       输出: DCMS_TOPIC_FL_OPT_SEND 收到二进制数据

步骤3: 模拟碰撞
  send(/dssad/trigger_debug/v1, [6, 0x01])
  预期: 日志 "COLLISON status: 1"
        DCMS_TOPIC_DSSAD_EVENT_CHECK 收到事件状态

步骤4: 模拟介入请求 (0→1)
  send(/dssad/trigger_debug/v1, [3, 0x01])
  预期: 事件触发，发送 EventId_InterventionRequest

步骤5: 模拟介入请求 (1→2, 递增触发)
  send(/dssad/trigger_debug/v1, [3, 0x02])
  预期: 再次触发 (last<cur 即 1<2)

步骤6: 关闭调试模式
  send(/dssad/trigger_debug/v1, [0xFF, 0x00])
  预期: 日志 "exit dssad debug"
```
